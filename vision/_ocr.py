...
class OCRHandler2:
    ...
    def _lazy_load_ocr_engine(self):
        ...
        # 设置绝对路径，存放到 (MODEL_BASE)/ocr/PP-OCRv4[/PP-OCRv4]
        engine_path = os.path.join(MODEL_OCR_PATH, "PP-OCRv4")
        os.makedirs(engine_path, exist_ok=True)

        # 支持两种布局：
        # 1) engine_path/ch_PP-OCRv4_*.onnx
        # 2) engine_path/PP-OCRv4/ch_PP-OCRv4_*.onnx  （huggingface subfolder）
        det_candidates = [
            os.path.join(engine_path, "ch_PP-OCRv4_det_infer.onnx"),
            os.path.join(engine_path, "PP-OCRv4", "ch_PP-OCRv4_det_infer.onnx"),
        ]
        rec_candidates = [
            os.path.join(engine_path, "ch_PP-OCRv4_rec_infer.onnx"),
            os.path.join(engine_path, "PP-OCRv4", "ch_PP-OCRv4_rec_infer.onnx"),
        ]

        det_path = next((p for p in det_candidates if os.path.exists(p)), None)
        rec_path = next((p for p in rec_candidates if os.path.exists(p)), None)

        # 如果本地不存在这两个文件，则执行下载（仍按 subfolder 规则）
